...
execSync(`cd ./res/testsuite && git checkout -b ${testsuiteCommitId} ${testsuiteCommitId}`);
...
fs.readdirSync('./res/testsuite').forEach(file => {
    ...
    if (fileStat.isFile() && file.endsWith('.wast')) {
        ...
        execSync(`cd ./res/testsuite && wast2json ${file} -o ./spectest/${fileBaseName}/${fileBaseName}.json`);
    }
});
